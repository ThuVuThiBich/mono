...
export const addWithdrawAddessRequest = async (params: {
  ...
  const { data } = await authRequest.post(`/user/asset/withdraw/newaddress`, params);
  return data;
};

export const getAllWithdrawAddressRequest = async (params: { coin_type: string }): Promise<IWithdrawAddress> => {
  const { data } = await authRequest.post(`/user/asset/withdraw/address`, params);
  return data;
};

export const sendWithDrawEmailRequest = async (params: ISendWithdrawEmailParams): Promise<any> => {
  const { data } = await authRequest.post(`/user/assets/confirm-withdrawal-on-network`, params);
  return data;
};

export const getWithDrawEmailCodeRequest = async (params: IGetWithdrawEmailParams): Promise<any> => {
  const { data } = await authRequest.post(`/user/assets/get-withdrawal-code`, params);
  return data;
};
...
export const confirmWithdrawRequest = async (params: {
  ...
  const { data } = await authRequest.post(`/user/asset/withdraw/confirm`, params);
  return data;
};

export const removeWithdrawAddressRequest = async (params: { type: string; index: number }) => {
  const { data } = await authRequest.post(`/asset/remove/withdraw/address`, params);
  return data;
};
...
export const getConvertCoinInfoRequest = async (params: IConvertCoinParams): Promise<IConvertCoinInfo> => {
  const { data } = await authRequest.post(`/bb/get/convertCoin`, params);
  return data;
};

export const getConvertCoinList = async (): Promise<string[]> => {
  const { data } = await authRequest.post(`bb/get/convertCoinList`);
  return data;
};

export const convertCoinRequest = async (params: IConvertCoinParams): Promise<any> => {
  const { data } = await authRequest.post(`bb/convert/coin`, params);
  return data;
};
